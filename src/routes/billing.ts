...
import { provisionNumberForUser } from "../services/telephony/TwilioNumberService.js";
...
billingRouter.post("/payg/calculate", async (request, response) => { const parsed = z.object({ minutes: z.coerce.number().int() }).safeParse(request.body); if (!parsed.success) return response.status(400).json({ error: "Minutes must be a whole number" }); try { response.json(await calculatePaygPrice(parsed.data.minutes)); } catch (error) { response.status(400).json({ error: error instanceof Error ? error.message : "Unable to calculate price" }); } });
